...
echo "MAKE sure that you edited datacard.txt which is used as input for the limit calculation!!!!!"
sleep 5
...
then
    ...
    if ! [ -f datacard.txt ]
	then
	echo "file datacard.txt needed as input is missing. Exiting ..."
	exit 0
    ...
    while [ $mass -le ${mass_max} ] 
      ...
      if [ -f $2/input_masses/Mass_${mass}_input/datacard.txt ]
	  then
	  rm -f $2/input_masses/Mass_${mass}_input/datacard.txt
      fi
      
      cp datacard.txt $2/input_masses/Mass_${mass}_input/tmp.txt
  ...
  #rates=`echo @rate_signal@`
	...
	rates=`echo "${rates} ${rates_tmp}"`
      ...
      rm -f tmp.txt
      echo "Now do sed"
      sed -e s/@root_input_file@/out_mass_${mass}.root/g -e  s/@rate_signal@/"${number_signal_events}"/g -e  s/@num_observed_events@/"${num_observed_events}"/g -e  s/@rates@/"${rates}"/g  $2/input_masses/Mass_${mass}_input/tmp.txt >> $2/input_masses/Mass_${mass}_input/datacard.txt
      
      cp $2/input_masses/Mass_${mass}_input/datacard.txt $2/output_masses/Mass_${mass}_output/datacard.txt
