...
then
    ...
    while [ $mass -le ${mass_max} ] 
      ...
      echo "cd /user/mukherjee/limits_Diboson/$1/output_masses/Mass_${mass}_output/" >> $1/output_masses/Mass_${mass}_output/run_limit_Mass_${mass}_expected.sh
      echo "cd /user/mukherjee/limits_Diboson/$1/output_masses/Mass_${mass}_output/" >> $1/output_masses/Mass_${mass}_output/run_limit_Mass_${mass}_observed.sh

      ##echo "combine -M MarkovChainMC -H ProfileLikelihood datacard.txt --tries ${n_tries_observed} -s -1  -m 50 -n observed > /dev/null" >> $1/output_masses/Mass_${mass}_output/run_limit_Mass_${mass}_observed.sh
     ...
     echo "combine -M MarkovChainMC -H ProfileLikelihood  datacard.txt --tries ${n_tries_observed} -s -1   -m 50 -n observed > /dev/null" >> $1/output_masses/Mass_${mass}_output/run_limit_Mass_${mass}_observed.sh
      ...
      do
      ##echo "combine -M MarkovChainMC -H ProfileLikelihood datacard.txt --tries ${n_tries_expected} -t ${n_toys_single} -s -1  > /dev/null" >> $1/output_masses/Mass_${mass}_output/run_limit_Mass_${mass}_expected.sh
      echo "combine -M MarkovChainMC  -H ProfileLikelihood   datacard.txt --tries ${n_tries_expected} -t ${n_toys_single} -s -1 --rMax 23.5 > /dev/null" >> $1/output_masses/Mass_${mass}_output/run_limit_Mass_${mass}_expected.sh
	let Nfiles=$Nfiles+1
      done
     ...
     if [ -e $1/output_masses/Mass_${mass}_output/condor/run_limit_condor_Mass_${mass}_observed.sh ]
      ...
      echo "cd /user/mukherjee/limits_Diboson/$1/output_masses/Mass_${mass}_output/condor" >> $1/output_masses/Mass_${mass}_output/condor/run_limit_condor_Mass_${mass}_expected.sh
      echo "cd /user/mukherjee/limits_Diboson/$1/output_masses/Mass_${mass}_output/condor" >> $1/output_masses/Mass_${mass}_output/condor/run_limit_condor_Mass_${mass}_observed.sh
      Nfiles=0
      while [ $Nfiles -le ${n_toys_files} ]
      do
